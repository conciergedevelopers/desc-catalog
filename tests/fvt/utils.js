...
exports.getrecords = function(recordName, callback) {
    exports.makeRestCall({method: 'GET'}, '/records', null, function(err, resp, body) {
        if(resp.statusCode === 200) {
            var returndocs = [];
            for(var i = 0; i < body.total_rows; i++) {
                if(body.rows[i].doc.name === recordName) {
                    returndocs.push(body.rows[i].doc);
                }
            }
            return callback(null, returndocs);
        } else {
            return callback("Failed to get records", null);
        }
    });
}